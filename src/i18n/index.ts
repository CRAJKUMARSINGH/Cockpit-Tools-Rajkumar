...
// Auto‑clean unsupported saved language
if (!supportedLanguages.includes(localStorage.getItem('app-language') ?? '')) {
...
function getSavedLanguage(): string {
  try {
    ...
    if (saved && !supportedLanguages.includes(resolveSupportedLanguage(saved))) {
      // stale/unsupported language, clear it
      localStorage.removeItem('app-language');
      return 'en';
    }
    return resolveSupportedLanguage(saved ? saved : 'en');
  } catch {
    return 'en';
  }
}
